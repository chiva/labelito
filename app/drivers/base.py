...
class Capability:
    ...
    two_color: bool
    # 600 dpi ("high resolution printing") raster support for the configured model. brother_ql's
    # own model table carries no such flag (see brother_ql.py's curated set) — a high_res=True
    # request on a model outside that set is rejected with a clean 4xx instead of silently sending
    # an unsupported raster bit.
    high_res: bool

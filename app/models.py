...
class RenderOptions(BaseModel):
    ...
    dither: bool | None = Field(
        ...
        examples=[None],
        description=(
            "Approximate greys with Floyd-Steinberg dithering. null inherits the DEFAULT_DITHER "
            "server default; true/false overrides it. On /print and /reprint this drives the "
            "actual print raster; /preview applies the identical conversion to its own pre-driver "
            "render (mirroring brother_ql's algorithm) so the preview looks like what will print. "
            "Inert when red=true on /print: brother_ql's two-color path separates layers by HSV "
            "filters and ignores dither; the server canonicalizes dither to false in the "
            "fingerprint and history under red — /preview never goes two-color, so this "
            "canonicalization does not apply there."
        ),
    )
    ...
    threshold: float | None = Field(
        ...
        description=(
            ...
            "server default (70.0). Drives /print and /reprint's raster; /preview applies the "
            "identical cutoff to its own pre-driver render so it looks like what will print. "
            "Inert when dither=true: brother_ql ignores threshold under Floyd-Steinberg dither; "
            "the server canonicalizes threshold to the default in the fingerprint and history "
            "when dither is on for /print (this canonicalization does not apply to /preview, "
            "which is not fingerprinted or persisted). Under red=true on /print, threshold IS "
            "applied (convert() runs a point() threshold on both the red and black layers after "
            "HSV separation) and is honored in the fingerprint and history."
        ),
    )
...
class CapabilityResponse(BaseModel):
    ...
    two_color: bool
    # 600 dpi ("high resolution") raster capability for the configured model — see the curated
    # model set in app/drivers/brother_ql.py. Clients discover this to decide whether to offer/
    # enable a `high_res` toggle.
    high_res: bool
...
class HealthResponse(BaseModel):
    status: str
    version: str  # installed package version (release-please bumps it every release)
...
class PrinterStatusResponse(BaseModel):
    ...
    model_mismatch: bool = Field(
        default=False,
        description="True when the device-reported model disagrees with the configured MODEL. The "
        "raster is built for MODEL at startup, so a mismatch means the output may be wrong for the "
        "attached printer. False when they agree or the device did not report a model.",
    )
    ...
    hostname: str | None = Field(
        default=None,
        description="Printer's self-reported network hostname (SNMP sysName), e.g. 'labelprinter'. "
        "SNMP-only: always None over USB (ESC i S) and the file transport, which have no analogue.",
    )

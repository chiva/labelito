...
def _resolve_fields(
    ...
    def replace(match: re.Match[str]) -> str:
        ...
        if key in ("date", "now"):
            ...
            return moment.strftime(fmt or (date_fmt if key == "date" else datetime_fmt))
        if key == "seq":
            return seq
...
class RenderEngine:
    ...
    def _resolve_all(
        ...
        return [
            self._resolve_element(el, fields, language, now, date_fmt, datetime_fmt, seq)
            for el in elements
        ]
    ...
    def _resolve_element(
        ...
        for attr in _TEMPLATED_ATTRS:
            ...
            if isinstance(raw, str):
                ...
                resolved[f"__{attr}__"] = _resolve_fields(
                    translated, fields, now, date_fmt, datetime_fmt, seq
                )
        ...
        if children:
            resolved["__children__"] = [
                self._resolve_element(child, fields, language, now, date_fmt, datetime_fmt, seq)
                for child in children
            ]
        return resolved

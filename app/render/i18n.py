...
This format is intentionally trivial so a translation platform (Weblate/Crowdin) can sync
to it later without code changes.
"""
...
def load_catalog(path: Path) -> dict[str, str]:
    """Load and validate a single ``<lang>.yaml`` catalog into a flat str→str mapping."""
    try:
...
class Translator:
    ...
    def date_formats(self, language: str) -> tuple[str, str]:
        ...
        for candidate in (self.default_language, lang):  # requested wins (applied last)
            ...
            if catalog is None:
                continue
            date_fmt = catalog.get(_DATE_FORMAT_KEY, date_fmt)
            datetime_fmt = catalog.get(_DATETIME_FORMAT_KEY, datetime_fmt)

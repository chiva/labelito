...
| `API_TOKEN` | *(unset)* | If set, all `/preview`, `/print*`, `/reprint/{job_id}`, and `/reload` endpoints require `Authorization: Bearer <token>`. The service **refuses to start** unless this, `WEB_AUTH_USER`/`WEB_AUTH_PASSWORD`, or `ALLOW_UNAUTHENTICATED` is set. Optional (bearer for scripts/automation) once HTTP Basic auth is configured — endpoints accept either credential. |

...
| `EDITOR_ENABLED` | `false` | YAML template studio visibility. `false` (default) makes `GET /editor`, `POST /preview/draft`, `POST /print/draft`, `POST /templates/parse`, `GET /templates/{name}/source`, and `POST /templates` return 404, hiding the studio entirely. Set `true` to enable the in-browser template editor, including printing the current draft directly (`POST /print/draft` — the raw YAML is printed without saving; render options inherit the `DEFAULT_*` settings unless set in the studio's Print options). Server-save (`POST /templates`) additionally requires `TEMPLATES_WRITABLE=true`. |
| `EDITOR_DEFAULT_MODE` | `visual` | Which authoring surface the studio opens in on page load: `visual` (the drag-and-drop block builder, the default) or `yaml` (the raw YAML editor). Only affects the initial surface — a toggle in the studio switches between them at runtime, and either mode keeps the same `#yaml` source of truth in sync. Requires `EDITOR_ENABLED=true`. |
| `TEMPLATES_WRITABLE` | `false` | Permit `POST /templates` to persist a draft YAML to `TEMPLATES_DIR` and hot-reload the registry. Default false because docker-compose mounts `templates/` read-only. Requires `EDITOR_ENABLED=true` — enabling writable without the editor still 404s the save route. If set `true` while `TEMPLATES_DIR` is not actually writable, the app logs a warning at boot and the save route returns a config-aware 500 that distinguishes the cause: a read-only mount (drop `:ro` from the templates volume) versus a host directory not writable by the container's uid/gid (chown it, or run with matching `UID`/`GID`). |
...
| `MCP_ENABLED` | `false` | Expose a **Model Context Protocol** server at `/mcp` (streamable HTTP) on the **same port/app** as the web UI, so an AI client (Claude Desktop, etc.) can generate/preview labels, print stored or **ephemeral** (designed-on-the-fly) labels, and reprint history labels. **Opt-in**: while disabled (default) the mount is absent (404). The endpoint reuses the **same auth** as the rest of the protected API — a valid `Authorization: Bearer <API_TOKEN>` or HTTP Basic — so it inherits the fail-closed startup guard. See [MCP server](mcp.md). |
| `MCP_WRITABLE` | `false` | Read-only vs read+write gate for the MCP tools (independent of `MCP_ENABLED`). Default `false`: only read-only tools are registered — list/get templates, capabilities, printer status, **preview** (stored & ephemeral), and history browse — none of which drive the printer. Set `true` to **also** register the write tools: `print_label` (stored template), `print_ephemeral_label` (inline label designed on the fly), and `reprint_history_label`. While `false`, a connected client never even sees the write tools in `tools/list`, so an AI cannot print by mistake. |
| `OIDC_ENABLED` | `false` | **Opt-in, additive** OAuth 2.0 Resource Server auth for `/mcp`. When `true`, `/mcp` **also** accepts a bearer **JWT** access token minted by an **external** OIDC provider (labelito publishes [RFC 9728](https://www.rfc-editor.org/rfc/rfc9728) Protected Resource Metadata and validates the token's signature via the issuer's JWKS, plus `iss`/`aud`/`exp`/scopes). Dynamic Client Registration + login happen at that provider — labelito is only a Resource Server. The static `API_TOKEN`/Basic keep working; OIDC covers **only** `/mcp`, not the REST API/web UI, and does **not** satisfy the fail-closed startup guard on its own (an OIDC-only deployment must still set `API_TOKEN`/`WEB_AUTH_*` or `ALLOW_UNAUTHENTICATED=true`). See [MCP server](mcp.md#oauth-20--oidc-authentication-external-idp). |
| `OIDC_ISSUER` | *(unset)* | Issuer URL exactly as it appears in the token `iss` claim (e.g. `https://idp.example.com/realms/labelito`); also advertised as the Authorization Server. **Required** when `OIDC_ENABLED=true` (must be `https://`, or `http://localhost` for dev). |
| `OIDC_AUDIENCE` | *(unset)* | Expected `aud` claim — the resource identifier you configure the IdP to mint `/mcp` tokens for (normally labelito's public `/mcp` URL). **Required** when `OIDC_ENABLED=true`. |
| `OIDC_REQUIRED_SCOPES` | *(unset)* | Space-separated scopes a token **must** carry (e.g. `labelito.print`); a token missing any gets `403 insufficient_scope`. Empty imposes no scope requirement. |
| `OIDC_JWKS_URI` | *(unset)* | Explicit JWKS endpoint; when set it **bypasses** OIDC discovery. |
| `OIDC_DISCOVERY` | `true` | When `OIDC_JWKS_URI` is unset, resolve the JWKS URI from `{issuer}/.well-known/openid-configuration`. |
| `OIDC_ALGORITHMS` | `RS256` | Space-separated allowlist of accepted JWT signing algorithms (set `ES256` for EC keys). A hard allowlist so `alg:none` and HMAC key-confusion are structurally rejected. |
| `OIDC_LEEWAY_SECONDS` | `60` | Clock-skew tolerance (seconds) applied to `exp`/`nbf`/`iat` during validation. |
...
| `FORWARDED_ALLOW_IPS` | `127.0.0.1` | Which **connection source IPs are trusted proxies** whose `X-Forwarded-Proto`/`-For`/`-Host` headers the server (uvicorn) honors. Matches on the **TCP peer IP**, not `X-Forwarded-For`. Accepts a comma-separated list of exact IPs, **CIDR networks** (e.g. `172.18.0.0/16`), or `*` (trust all). The default trusts only literal `127.0.0.1` — which a **containerized** proxy never appears as (Docker rewrites the source to the container/gateway IP), so forwarded headers are **ignored** and any redirect the app emits (such as the MCP `/mcp` → `/mcp/` 307) is built as `http://` and breaks behind a TLS proxy. Set this to the source address the proxy connects from (the peer IP labelito sees) — the proxy's container subnet or the Docker bridge gateway — so scheme-aware redirects and generated URLs stay `https`. Only trust proxies that **overwrite** these headers on every request, and never trust a range from which labelito is directly reachable (a client could then spoof its scheme/IP). To disable proxy-header trust entirely, run uvicorn with `--no-proxy-headers`. See [reverse-proxy deployment](reverse-proxy.md). |
...
- **Trusting a TLS-terminating proxy.** When a proxy terminates TLS and forwards over plain HTTP,
  set `FORWARDED_ALLOW_IPS` to the proxy's IP or CIDR so labelito honors `X-Forwarded-Proto: https`
  and keeps generated URLs and redirects (including the MCP `/mcp` → `/mcp/` 307) on `https`. The
  default (`127.0.0.1`) ignores forwarded headers from a containerized proxy. Full walkthrough with
  per-proxy examples: [reverse-proxy deployment](reverse-proxy.md).

...
1. **Media mismatch.** Printer has **62mm continuous** tape loaded. Template `62x29-address.yaml`
   (then filed as `address.yaml`, before the media-prefixed template rename) requests
...
- **Acceptance:** `tests/test_drivers.py` — `"62"`→62mm continuous, `"62x29"`→62×29 die-cut; vs
  loaded-62-continuous fixture: `62x29-address.yaml`⇒mismatch, others⇒match, SNMP-down⇒unknown.
...
- Manual vs the real printer (`192.168.5.14`, 62mm continuous loaded):
  ...
  - `62x29-address.yaml` (62x29) ⇒ **409**; option disabled in print dropdown; Print button disabled; badge

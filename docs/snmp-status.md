...
- **Model cross-check.** The driver capability table and raster are built from `MODEL` at startup, so
  a status read whose reported model disagrees with `MODEL` means the raster may be wrong for the
  attached device. On any reachable read (SNMP or USB), the reported model is compared to `MODEL`
  (normalized, so the `Brother QL-810W` descriptor matches `QL-810W`); a confident conflict logs a
  warning **once per distinct reported model** and surfaces as `model_mismatch: true` on
  `/printer/status`. It is **advisory only** — like the fail-open guard, it never blocks a print, and
  an unknown/unreported model never flags.

...
def test_capabilities_reports_high_res(client: TestClient) -> None:
    """/capabilities surfaces high_res so clients can discover 600 dpi support."""
    resp = client.get("/capabilities")
    assert resp.status_code == 200
    assert resp.json()["high_res"] is True  # QL-810W (the test default model) is 600dpi-capable


def test_print_page_high_res_hint_when_supported(client: TestClient) -> None:
    """On a high-res-capable model the checkbox is enabled and carries the ordinary hint text."""
    html = client.get("/").text
    assert 'id="high-res" disabled' not in html
    assert "improves small text" in html
    assert "not supported by" not in html


def test_high_res_true_on_unsupported_model_rejected_422(
    client: TestClient, monkeypatch: pytest.MonkeyPatch
) -> None:
    """high_res=true when the configured model lacks 600 dpi support is a clean 422, not a 500."""
    import app.main as main_mod
    from app.drivers.brother_ql import BrotherQLDriver

    monkeypatch.setattr(main_mod.settings, "model", "QL-500")  # not in the curated high-res set
    monkeypatch.setattr(main_mod, "_driver_cls", BrotherQLDriver.for_model("QL-500"))
    resp = client.post(
        "/print",
        json={
            "template": "simple",
    ...
    assert "QL-500" in resp.text


def test_preview_high_res_true_on_unsupported_model_rejected_422(
    client: TestClient, monkeypatch: pytest.MonkeyPatch
) -> None:
    """/preview applies the same high_res capability gate as /print (both mirror the same helper)."""
    import app.main as main_mod
    from app.drivers.brother_ql import BrotherQLDriver

    monkeypatch.setattr(main_mod.settings, "model", "QL-500")
    monkeypatch.setattr(main_mod, "_driver_cls", BrotherQLDriver.for_model("QL-500"))
    resp = client.post(
        "/preview",
        json={
...
def test_preview_high_res_false_on_unsupported_model_still_200(
    client: TestClient, monkeypatch: pytest.MonkeyPatch
) -> None:
    """The high_res gate only fires when high_res is actually requested True; omitting it (or false)
    on an unsupported model must not block an otherwise-ordinary preview."""
    import app.main as main_mod
    from app.drivers.brother_ql import BrotherQLDriver
...
def test_print_high_res_true_on_unsupported_model_still_422_regardless_of_env_default(
    client: TestClient, monkeypatch: pytest.MonkeyPatch
) -> None:
    """An explicit high_res=true is rejected even when DEFAULT_HIGH_RES=false — the client asked for
    it explicitly, resolving to True either way."""
    import app.main as main_mod
    from app.drivers.brother_ql import BrotherQLDriver
...
def test_print_page_high_res_disabled_and_hinted_for_unsupported_model(
    client: TestClient, monkeypatch: pytest.MonkeyPatch
) -> None:
    """On a model without 600 dpi support the print page renders the checkbox disabled, unchecked
    (even with DEFAULT_HIGH_RES=true — a disabled control must never silently read back as checked),
    and shows a hint naming the model instead of the ordinary description."""
    import app.main as main_mod
    from app.drivers.brother_ql import BrotherQLDriver
    ...
    monkeypatch.setattr(main_mod.settings, "default_high_res", True)
    html = client.get("/").text
    assert 'id="high-res" disabled' in html
    assert 'id="high-res" checked' not in html, (
        "a disabled checkbox must not also render checked — the client would still submit high_res:true"
    )
    assert "not supported by QL-500" in html


def test_print_request_schema_documents_high_res_capability(client: TestClient) -> None:
    """`high_res` must surface in the CapabilityResponse schema alongside two_color."""
    spec = client.get("/openapi.json").json()
    schemas = spec["components"]["schemas"]
    assert "high_res" in schemas["CapabilityResponse"]["properties"]

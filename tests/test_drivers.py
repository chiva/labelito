...
# ── capability surfaces 600 dpi high-res support (curated, not derived from the fork) ──────────────
def test_capability_high_res_models_are_the_curated_eight() -> None:
    """Exactly the models Brother's Raster Command Reference documents the high-res bit for.

    Verified against Brother's per-series Raster Command Reference PDFs (the installed brother_ql
    fork carries no such flag on ALL_MODELS, unlike two_color/cutting — see the curated set's comment
    in app/drivers/brother_ql.py for the per-series citations).
    """
    high_res_models = {m for m in _FORK_MODELS if BrotherQLDriver.for_model(m).CAPABILITY.high_res}
    assert high_res_models == {
        "QL-570",
        "QL-580N",
        "QL-700",
        "QL-710W",
...
@pytest.mark.parametrize(
    "model_id",
    ["QL-500", "QL-550", "QL-560", "QL-600", "QL-650TD", "QL-1050", "QL-1060N", "QL-1100"],
)
def test_capability_high_res_false_for_uncurated_models(model_id: str) -> None:
    """Models outside the curated high-res set report high_res=False (fail closed, not guessed)."""
    assert BrotherQLDriver.for_model(model_id).CAPABILITY.high_res is False
